...
/* @brief Pointer to FPGA control registers. -------------------------------------------Fenske */
static osc_fpga_reg_mem_t *g_asg_fpga_reg_mem = NULL;
...
{
    void *page_ptr, *page_ptr2;
    long page_addr, page_off, page_addr2, page_off2, page_size = sysconf(_SC_PAGESIZE);
    ...
    /*--------------------------------------------------------------------------Fenske-*/
    page_addr2 = ASG_FPGA_BASE_ADDR & (~(page_size-1));
    page_off2  = ASG_FPGA_BASE_ADDR - page_addr2;

    page_ptr2 = mmap(NULL, OSC_FPGA_BASE_SIZE, PROT_READ | PROT_WRITE,
                          MAP_SHARED, g_osc_fpga_mem_fd, page_addr2);
  ...
  /*  g_osc_fpga_reg_mem = page_ptr2 + page_off;
    ...
    /* ----------------------------------------------------------------------------Fenske */
    g_asg_fpga_reg_mem = page_ptr2 + page_off2;
    g_osc_fpga_cho_mem = (uint32_t *)g_asg_fpga_reg_mem +
        (OSC_FPGA_CHA_OFFSET/ sizeof(uint32_t));

   /* int *banane = (int *) g_osc_fpga_cho_mem;
    banane = 0;
    *banane =5;
   */
...
/*    int fd = 0;								// File descriptor
    	...
    	fd = open("/dev/mem", O_RDWR | O_SYNC);	// bildet den Inhalt der Datei "mem" im Arbeitsspeicher ab
    	unsigned long offset = 0x40300000;		// Offset des mapped-Speicherbereiches zB 0x40000000 für Housekeeping
    	* Map one page (4096 bytes von 0x4xxx xxxx beginnend) *
    	map_base = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset & ~MAP_MASK);

    //	write_values(0x30, access_type, wert, val_count);		// Beispiel Wert schreiben
    	while(1){
    		printf("P-Teil: %d I-Teil: %d D-Teil: %d\n",  read_value(0x5C), read_value(0X60), read_value(0x64));
    	}
                           ...
                           int enable_avg_at_dec)
{
    ...
    // *g_asg_fpga_reg_mem = *g_osc_fpga_reg_mem; // ------------------------------- Fenske
 ...
 * @retval 0 Success, never fails
 */
int osc_fpga_get_sig_ptr(int **cha_signal, int **chb_signal) //, int **cho_signal)
{
    *cha_signal = (int *)g_osc_fpga_cha_mem;
    *chb_signal = (int *)g_osc_fpga_chb_mem; //my_map_base + 0x10000; //g_osc_fpga_chb_mem;
    //*cho_signal = (int *)g_osc_fpga_cho_mem; //  ----------------- Fenske
    return 0;
}
